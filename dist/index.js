...
import { z } from "zod";
import { addInsightToDashboard, createDashboard, createFeatureFlag, createInsight, deleteDashboard, deleteFeatureFlag, deleteInsight, errorDetails, getDashboard, getDashboards, getFeatureFlagDefinition, getFeatureFlags, getInsight, getInsights, getLLMTotalCostsForProject, getOrganizationDetails, getOrganizations, getProjects, getPropertyDefinitions, getSqlInsight, getUser, listErrors, updateDashboard, updateFeatureFlag, updateInsight, } from "./posthogApi";
import { AddInsightToDashboardSchema, CreateDashboardInputSchema, ListDashboardsSchema, UpdateDashboardInputSchema, } from "./schema/dashboards";
import { FilterGroupsSchema, UpdateFeatureFlagInputSchema } from "./schema/flags";
import { CreateInsightInputSchema, ListInsightsSchema, UpdateInsightInputSchema, } from "./schema/insights";
...
export class MyMCP extends McpAgent {
    ...
    get cache() {
        if (!this._cache) {
            this._cache = new DurableObjectCache(this.requestProperties.userHash, this.ctx.storage);
        }
        return this._cache;
    }
    async getDistinctId() {
    ...
    async init() {
        ...
        this.registerTool("organization-details-get", `
            ...
            try {
                const orgId = await this.getOrgID();
                const organizationDetails = await getOrganizationDetails(orgId, this.requestProperties.apiToken);
        ...
        this.registerTool("create-feature-flag", `Creates a new feature flag in the project. Once you have created a feature flag, you should:
            ...
            const featureFlagWithUrl = {
                ...featureFlag,
                url: `${getProjectBaseUrl(projectId)}/feature_flags/${featureFlag.id}`,
            };
            return {
        ...
        this.registerTool("insights-get-all", `
            ...
            try {
                const projectId = await this.getProjectId();
                const insights = await getInsights(projectId, this.requestProperties.apiToken, data);
        ...
        this.registerTool("insight-get", `
            ...
            try {
                const projectId = await this.getProjectId();
                const insight = await getInsight(projectId, insightId, this.requestProperties.apiToken);
        ...
        this.registerTool("dashboards-get-all", `
            ...
            try {
                const projectId = await this.getProjectId();
                const dashboards = await getDashboards(projectId, this.requestProperties.apiToken, data);
        ...
        this.registerTool("dashboard-get", `
            ...
            try {
                const projectId = await this.getProjectId();
                const dashboard = await getDashboard(projectId, dashboardId, this.requestProperties.apiToken);
